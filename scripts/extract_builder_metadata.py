...
def find_zetta_utils_package():
    ...
    try:
        import zetta_utils
        package_path = Path(zetta_utils.__file__).parent
        print(f"Found zetta_utils package at: {package_path}")
        return package_path
    except ImportError:
        print("Error: zetta_utils package not found. Please ensure it's installed in the current Python environment.")
        sys.exit(1)

def make_path_relative_to_package(absolute_path: str, package_path: Path) -> str:
    """Convert absolute path to relative path from zetta_utils package root."""
    try:
        abs_path = Path(absolute_path)
        if package_path in abs_path.parents or abs_path == package_path:
            relative_path = abs_path.relative_to(package_path.parent)
...
package_path = find_zetta_utils_package()
...
try:
    ...
    print(f"Error importing zetta_utils: {e}")
...
def extract_type_info(annotation) -> Dict[str, Any]:
    ...
    if origin is typing.Literal:
        ...
        for arg in args:
            ...
            elif hasattr(arg, '__origin__') and hasattr(arg, '__args__'):
                inner_origin = get_origin(arg)
                inner_args = get_args(arg)
                if inner_origin and inner_args:
    ...
    if origin is not None:
        ...
        if hasattr(origin, '__name__'):
            ...
            if any(seq_name in origin_name for seq_name in ['sequence', 'list', 'tuple', 'iterable']):
                is_sequence = True
                if args:
...
def unwrap_decorated_function(fn: callable) -> callable:
    ...
    while depth < max_unwrap_depth:
        ...
        if hasattr(original_fn, 'fn') and callable(getattr(original_fn, 'fn', None)):
            ...
            if 'DictSupportingTensorOp' in class_name or hasattr(original_fn, '__call__'):
                original_fn = original_fn.fn
                unwrapped_something = True
                continue
        
        # Handle other common wrapper patterns
        if hasattr(original_fn, '_wrapped_fn') and callable(getattr(original_fn, '_wrapped_fn', None)):
            original_fn = original_fn._wrapped_fn
            unwrapped_something = True
            continue
        
        # Handle functools.partial objects
        if hasattr(original_fn, 'func') and callable(getattr(original_fn, 'func', None)):
        ...
        if hasattr(original_fn, '__closure__') and original_fn.__closure__:
            ...
            for cell in original_fn.__closure__:
                if cell.cell_contents and callable(cell.cell_contents):
                    ...
                    if (hasattr(cell_fn, '__name__') and 
                        cell_fn.__name__ != original_fn.__name__ and
                        cell_fn.__name__ != 'wrapped' and
                        not cell_fn.__name__.startswith('_')):
                        original_fn = cell_fn
                        unwrapped_something = True
                        break
...
def extract_function_signature(fn: callable) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not get type hints for {getattr(original_fn, '__name__', str(original_fn))}: {e}")
            type_hints = {}
        ...
        for param_name, param in sig.parameters.items():
            ...
            try:
                ...
                print(f"Warning: type_info for {param_name} not JSON serializable: {e}")
                # Convert any non-serializable values to strings
                param_info["type_info"] = {k: str(v) if v is not None else None for k, v in type_info.items()}
        ...
        return {
            "parameters": {},
            "return_type": "Any",
            "error": f"Failed to extract signature: {str(e)}"
        }
    ...
    for builder_name, entries in REGISTRY.items():
        ...
        for entry in entries:
            ...
            try:
                ...
                file_path = make_path_relative_to_package(file_path, package_path)
            except (TypeError, OSError):
                file_path = "unknown"
            
            try:
                line_number = original_fn.__code__.co_firstlineno if hasattr(original_fn, "__code__") else 0
...
def convert_to_extension_format(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def version_spec_to_suffix(version_spec: str) -> str:
        ...
        if version_spec == ">=0.0.0" or version_spec.startswith(">="):
            ...
            return "_v" + version_spec.replace(".", "_").replace(">", "gt").replace("<", "lt").replace("=", "eq")
    ...
    for builder_name, builder_info in metadata["builders"].items():
        ...
        for version_info in builder_info["versions"]:
            ...
            extension_metadata.append({
                ...
                "version_spec": version_info["version_spec"],  # Store original version spec
                "metadata": {
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract zetta_utils builder metadata for VSCode extension')
    parser.add_argument('--output-dir', type=str, help='Output directory for metadata files (defaults to temp directory)')
    ...
    try:
        ...
        package_info = {
            "zetta_utils_package_path": str(package_path)
        }
        ...
        if args.verbose:
            print(f"✅ Successfully extracted metadata for {metadata['statistics']['total_builders']} builders")
            print(f"   Total entries: {metadata['statistics']['total_entries']}")
            print(f"   With partial support: {metadata['statistics']['builders_with_partial']}")
            print(f"   With parallel support: {metadata['statistics']['builders_with_parallel']}")
